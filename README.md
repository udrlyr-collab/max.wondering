...
MovieMax는 CGV IMAX 상영 회차를 주기적으로 조회하고, 새 예매 회차·예매 오픈·추적 중인 회차의 잔여석 증가를 Telegram으로 알리는 개인용 관리 콘솔입니다. 관리자 화면에서는 날짜, 시작·종료 시각, 상영관, 예매 상태, 잔여석/전체 좌석을 확인할 수 있습니다.

기본 설정은 CGV 용산아이파크몰의 `오디세이` IMAX 대상을 처음 데이터베이스에 생성합니다. 콘솔에서 다른 CGV 극장과 현재 조회되는 IMAX 영화도 추가할 수 있습니다.
...
- CGV 극장과 현재 조회되는 IMAX 영화를 선택해 감시 대상 추가
- 감시 대상 활성화/비활성화
- 새 IMAX 회차 및 예매 오픈 알림 활성화/비활성화
...
| `CGV_FORMAT_KEYWORD`, `CGV_SCREEN_GRADE_CODE` | IMAX 회차 필터. 현재 예시는 `IMAX`, `0301`입니다. |
...
6. 알림 받을 채팅을 선택하고 `Telegram 알림 활성화`를 켠 뒤 `설정 저장`을 누릅니다.
...
이 경로들은 이 프로젝트를 위한 공식·계약형 API가 아닙니다. CGV가 URL, 요청 파라미터, 응답 필드, IMAX 구분 코드, 예매 상태 의미 또는 접근 정책을 바꾸면 감시가 중단되거나 코드 수정이 필요할 수 있습니다. Cloudflare 또는 CGV가 Lightsail 출발 IP를 차단하거나 제한하면 HTTP 403/429가 발생할 수 있으며, 로컬 PC에서 성공했다는 사실만으로 Lightsail에서도 성공한다고 보장할 수 없습니다.
